...
impl OledDisplay {
    ...
    /// The `image_data` should be raw binary data for a 128x64 display.
    pub fn splash(&mut self, show_splash: bool) -> Result<(), Box<dyn std::error::Error>> {
        self.display.clear_buffer();
        if show_splash {
            ...
            // 2. Clear display and draw the splash image
            self.clear();
            let raw_image = ImageRaw::<BinaryColor>::new(imgdata::SPLASH_IMAGE_RAW_DATA, constants::SPLASH_WIDTH);
            ...
            // 3. Fade in brightness over 2.5 seconds
            const FADE_DURATION_MS: u64 = 2500;
            const FADE_STEPS: u8 = 50; // More steps for smoother fade
    ...
    fn get_text_width(&self, text: &str) -> u32 {
        if let Some(ref font) = self.custom_font {
            ...
            // A simpler way is to use layout.width() if available, but it often needs more context.
            // For a single line, we can sum the advance_width.
    ...
    fn draw_text_with_custom_font(
        ...
        if let Some(ref font) = self.custom_font {
            ...
            //let mut x_cursor = top_left.x;
            let mut pixels_to_draw: Vec<Pixel<BinaryColor>> = Vec::new();

            // why not render then draw_iter???
            for letter in text.chars() {

                let (metrics, bitmap) = font.rasterize(letter, constants::MAIN_FONT_HEIGHT as f32);

                for y in 0..metrics.height {
                    for x in 0..metrics.width {
                        let alpha = bitmap[y * metrics.width + x];
                        if alpha > 128 { // Use a threshold for alpha to determine 'on' pixel
                            // Calculate the absolute position on the display
                            let px_x = (top_left.x as f32 + metrics.advance_width.round() as f32 + x as f32).round() as i32;
                            let px_y = (top_left.y as f32 + metrics.advance_height.round() as f32 + y as f32).round() as i32;

                            let target_point = Point::new(px_x, px_y);

                            if target_point.x >= 0 && target_point.x < constants::DISPLAY_WIDTH as i32 &&
                               target_point.y >= 0 && target_point.y < constants::DISPLAY_HEIGHT as i32 {
                                pixels_to_draw.push(Pixel(target_point, BinaryColor::On));
                            }
                        }
                    }
                }
            ...
            // Draw all collected pixels in one go
            self.display.draw_iter(pixels_to_draw.into_iter())
                .map_err(|e| Box::new(DisplayDrawingError::from(e)) as Box<dyn std::error::Error>)?;

        } else {
            // Fallback to default mono font
            Text::with_baseline(
                text,
                top_left,
                self.default_mono_style,
                _baseline,
            )
            .draw(&mut self.display)
            .map_err(|e| Box::new(DisplayDrawingError::from(e)) as Box<dyn std::error::Error>)?;
    ...
    pub fn set_status_line_data(&mut self, volume_percent: u8, is_muted: bool, samplesize: String, samplerate: String, repeat_mode: RepeatMode, shuffle_mode: ShuffleMode)
    {
        ...
        if changed {
            ...
            info!("Status line data changed. Updating internal state.");
            self.volume_percent = volume_percent;
            self.is_muted = is_muted;
            self.repeat_mode = repeat_mode;
            self.shuffle_mode = shuffle_mode;
            
            self.bitrate_text = if samplesize == "1" { // DSD/DSF 1-bit
                format!("DSD{} ", 
                    (samplerate.parse::<u32>().unwrap_or(0) / 44100 as u32))
            } else { // vanilla, e.g. 24/96 etc
                format!("{}/{} ", 
                    samplesize, 
                    (samplerate.parse::<u32>().unwrap_or(0) / 1000 as u32))
            }
            .to_string();
                                
            self.audio_bitrate = if self.bitrate_text.to_uppercase().contains("DSD") {
                AudioBitrate::DSD
            } else if samplesize == "24" || samplerate.parse::<u32>().unwrap_or(0) >= 44100 {
                AudioBitrate::HD
            } else if !self.bitrate_text.is_empty() { // Default to SD if just a normal bitrate is present
                AudioBitrate::SD
            } else {
                AudioBitrate::None
            };
    ...
    pub fn update_and_draw_clock(&mut self, current_time: chrono::NaiveTime) -> Result<(), Box<dyn std::error::Error>> {
        ...
        if current_date_string != self.last_drawn_date {
            ...
            Text::with_baseline(
                &current_date_string,
                Point::new(date_x_pos, date_y),
                MonoTextStyleBuilder::new().font(&FONT_6X10).text_color(BinaryColor::On).build(), // Use FONT_6X10 for date
                Baseline::Top,
            )
            .draw(&mut self.display)
            .map_err(|e| Box::new(DisplayDrawingError::from(e)) as Box<dyn std::error::Error>)?;
    ...
    pub fn render_frame(&mut self) -> Result<(), Box<dyn std::error::Error>> {
        match self.current_mode {
            ...
            DisplayMode::Scrolling => {
                ...
                let vol_text_width = self.get_text_width(&vol_text) as i32;
                Text::with_baseline(
                    &vol_text,
                    Point::new(current_x, constants::DISPLAY_REGION_Y_OFFSET),
                    self.default_mono_style,
                    Baseline::Top,
                )
                .draw(&mut self.display)
                ...
                current_x += vol_text_width; // Move X past the volume text
                debug!("Current X after volume: {}", current_x);
                ...
                Text::with_baseline(
                    &self.bitrate_text,
                    Point::new(right_block_x, constants::DISPLAY_REGION_Y_OFFSET),
                    self.default_mono_style,
                    Baseline::Top,
                )
                .draw(&mut self.display)
                ...
                if info_line_changed {
                    ...
                    // Draw current time (left-justified)
                    Text::with_baseline(
                        &current_time_str,
                        Point::new(constants::DISPLAY_REGION_X_OFFSET, info_line_y),
                        self.default_mono_style,
                    ...
                    Text::with_baseline(
                        &self.mode_text,
                        Point::new(mode_text_x, info_line_y),
                        self.default_mono_style,
                        Baseline::Top,
                    )
                    .draw(&mut self.display)
                    ...
                    Text::with_baseline(
                        &remaining_time_str,
                        Point::new(remaining_time_x-3, info_line_y),
                        self.default_mono_style,
                        Baseline::Top,
                    )
                    .draw(&mut self.display)
